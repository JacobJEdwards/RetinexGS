...
def cal_min_max(
    normal: Tensor, normal_sign: Tensor, bias: Tensor
) -> tuple[Tensor, Tensor]:
    normal_max = (
        normal[:, 0, :, :] * normal_sign[:, 0, :, :]
        + normal[:, 1, :, :] * normal_sign[:, 1, :, :]
        + normal[:, 2, :, :] * normal_sign[:, 2, :, :]
        + bias
    )
    normal_min = (
        normal[:, 0, :, :] * (1 - normal_sign[:, 0, :, :])
        + normal[:, 1, :, :] * (1 - normal_sign[:, 1, :, :])
        + normal[:, 2, :, :] * (1 - normal_sign[:, 2, :, :])
        + bias
    )
...
def pixel_project(img, normal, normal2, bias):
    ...
    normal2 = torch.stack(
        [
            -(normal2[:, 0] * normal[:, 1] + normal2[:, 1] * normal[:, 2])
            / normal[:, 0],
            normal2[:, 0],
            normal2[:, 1],
        ],
    ...
    normal3 = torch.stack(
        [
            normal[:, 1] * normal2[:, 2] - normal[:, 2] * normal2[:, 1],
    ...
    t1 = (
        ...
        + G * normal[:, 1, :, :]
        + B * normal[:, 2, :, :]
        + bias[:, 0].unsqueeze(-1).unsqueeze(-1)
    )
    t2 = (
        R * normal2[:, 0, :, :]
        + G * normal2[:, 1, :, :]
        + B * normal2[:, 2, :, :]
        + bias[:, 1].unsqueeze(-1).unsqueeze(-1)
    )
    t3 = (
        R * normal3[:, 0, :, :]
        + G * normal3[:, 1, :, :]
        + B * normal3[:, 2, :, :]
        + bias[:, 2].unsqueeze(-1).unsqueeze(-1)
    )
    ...
    t1_max, t1_min = cal_min_max(
        normal, normal_sign, bias[:, 0].unsqueeze(-1).unsqueeze(-1)
    )

    t2_max, t2_min = cal_min_max(
        normal2, normal2_sign, bias[:, 1].unsqueeze(-1).unsqueeze(-1)
    )

    t3_max, t3_min = cal_min_max(
        normal3, normal3_sign, bias[:, 2].unsqueeze(-1).unsqueeze(-1)
    )
    ...
    return (
        [t1, normal, t1_max, t1_min],
        [t2, normal2, t2_max, t2_min],
        [t3, normal3, t3_max, t3_min],
        bias,
    )
