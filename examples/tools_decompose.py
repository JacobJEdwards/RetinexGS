...
def pixel_project_2d(img, normal, normal2, t_min, t_max, distance_max):
    ...
    distance = (
        vector[:, :, 0] ** 2 + vector[:, :, 1] ** 2 + vector[:, :, 2] ** 2 + 1e-8
    ) ** 0.5
    ...
    normal2 = torch.stack(
        [
            normal2[0],
            normal2[1],
            -(normal2[0] * normal[0] + normal2[1] * normal[1]) / normal[2],
        ]
    )
    normal3 = torch.stack(
        [
    ...
    phase = torch.stack(
        [
            (vector[:, :, 1] * normal2[2] - vector[:, :, 2] * normal2[1]),
            (vector[:, :, 2] * normal2[0] - vector[:, :, 0] * normal2[2]),
            (vector[:, :, 0] * normal2[1] - vector[:, :, 1] * normal2[0]),
        ],
        dim=-1,
    )
...
def pixel_project_back(normals, ts, distances, cos_theta, sin_theta):
    ...
    img_2D = (
        0.5
        + (distance * cos_theta).unsqueeze(-1) * normal2
        + (distance * sin_theta).unsqueeze(-1) * normal3
    )
