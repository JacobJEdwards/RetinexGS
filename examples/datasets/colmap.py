...
class Parser:
    ...
    def __init__(
        ...
        colmap_dir = os.path.join(data_dir, "colmap_sparse/0/") # Adapte to LOM dataset
        if not os.path.exists(colmap_dir):
            colmap_dir = os.path.join(data_dir, "colmap_sparse") # Adapte to LOM dataset
        assert os.path.exists(
            colmap_dir
        ), f"COLMAP directory {colmap_dir} does not exist."

        manager = SceneManager(colmap_dir)  # package of pycolmap
        ...
        inds = id_train + id_val
        # print('training scene number', len(id_train))
        # print('validation scene number', len(id_val))

        image_names = [image_names[i] for i in inds]
        camtoworlds = camtoworlds[inds]
        camera_ids = [camera_ids[i] for i in inds]

        colmap_image_dir = os.path.join(data_dir, exp_name)
        image_dir = os.path.join(data_dir, exp_name)
        ...
        self.lentrain = len(id_train)
        self.lenval = len(id_val)
...
class Dataset:
    ...
    def __init__(
        ...
        if split == "train":
            ...
            self.indices = indices[:self.parser.lentrain]
        else:
            #self.indices = indices[indices % self.parser.test_every == 0]   # 划分train & val set注意和LOM保持一致
            self.indices = indices[self.parser.lentrain: self.parser.lentrain+self.parser.lenval]
    ...
    def __getitem__(self, item: int) -> Dict[str, Any]:
        ...
        # print('cuiziteng', self.parser.image_paths)
        if self.split == 'train':
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", type=str, default="/data/utsubo0/users/cui/data/LOM/buu")
