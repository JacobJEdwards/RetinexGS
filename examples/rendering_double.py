...
def rasterization_dual(
    ...
    if distributed:
        if packed:
            ...
            (means2d, depths, conics, opacities, colors, colors_low) = (
                all_to_all_tensor_list(
                    world_size,
                    [means2d, depths, conics, opacities, colors, colors_low],
                    cnts,
            ...
            offsets = torch.cumsum(
                torch.tensor([0] + C_world[:-1], device=device, dtype=torch.long), dim=0
            )
            camera_ids = camera_ids - offsets.repeat_interleave(torch.stack(cnts))
            offsets = torch.cumsum(
                torch.tensor([0] + N_world[:-1], device=device, dtype=torch.long), dim=0
            )
            ...
            (radii,) = all_to_all_tensor_list(
                world_size,
                [radii.flatten(0, 1)],
                [C_i * N for C_i in C_world],
                [C * N_i for N_i in N_world],
            )
            ...
            means2d, depths, conics, opacities = (
                reshape_view(C, means2d, N_world),
                reshape_view(C, depths, N_world),
                reshape_view(C, conics, N_world),
                reshape_view(C, opacities, N_world),
            )
            colors, colors_low = (
                reshape_view(C, colors, N_world),
                reshape_view(C, colors_low, N_world),
            )
    ...
    if render_mode in ["RGB+D", "RGB+ED"]:
        ...
        if backgrounds is not None:
            backgrounds = torch.cat(
                [backgrounds, torch.zeros(batch_dims + (C, 1), device=device)], dim=-1
            )
        ...
        for i in range(n_chunks):
            ...
            if with_eval3d:
                raise NotImplementedError(
                    "Chunked dual rendering for `with_eval3d` is not implemented."
                )
            else:
